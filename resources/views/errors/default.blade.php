...
<head>
    ...
    <title>Kesalahan - KeuanganKu</title>
...
<body class="bg-background text-text-primary">
    <div class="min-h-screen flex items-center justify-center px-4">
        <div class="max-w-lg w-full text-center">
            ...
            <!-- Error Status -->
            <div class="mb-6">
                <h1 class="text-6xl font-bold text-accent mb-2">{{ $status ?? 'Error' }}</h1>
                <div class="h-1 w-24 bg-accent mx-auto mb-4"></div>
            </div>

            <!-- Error Message -->
            <div class="mb-8">
                <h2 class="text-3xl font-bold mb-3">{{ $message ?? 'Terjadi Kesalahan' }}</h2>
                <p class="text-text-secondary text-lg">
                    Maaf, terjadi kesalahan saat memproses permintaan Anda. Silakan coba lagi nanti atau hubungi dukungan kami jika masalah berlanjut.
                </p>
            </div>

            <!-- Illustration -->
            <div class="mb-8 p-8 bg-surface-secondary rounded-2xl border border-border-light">
                <svg class="w-24 h-24 mx-auto text-accent opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <a href="{{ route('dashboard') }}" class="px-6 py-3 bg-accent text-black font-semibold rounded-lg hover:bg-accent-dark transition">
                    Ke Dashboard
                </a>
            </div>
